    ...
    <div className="min-h-screen bg-white flex flex-col items-center justify-between px-7 py-12">
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-sm">
        ...
        <div className="fade-up-2 w-full space-y-3 mb-10">
          {[
            { icon: <Camera className="w-4 h-4" style={{color:'#1a8a3c'}}/>, text: "4 required photos: plate, chassis & 2 sides" },
...
function CameraCapture({ step, stepIndex, totalRequired, isExtra, extraLabel, onCapture, onBack }) {
  ...
  const instruction = isExtra ? "Capture any additional damage or details" : step?.instruction;
  ...
  return (
    <div className="h-screen bg-black flex flex-col relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 z-10 bg-gradient-to-b from-black/70 to-transparent px-5 pt-5 pb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="flex-1">
            <span className="text-white font-syne font-bold text-base block" style={{fontWeight:700}}>{label}</span>
            {isExtra && (
              <span className="text-orange-300 text-xs">Additional Photo</span>
            )}
          </div>
          {!isExtra && (
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 bg-gradient-to-t from-black/80 to-transparent px-5 pt-8 pb-8">
        <p className="text-white/70 text-xs text-center mb-4">{instruction}</p>
        <button onClick={capture} disabled={!streamReady || !orientationOk}
          ...
          {isExtra ? "Capture Additional Photo" : `Capture ${label} (${stepIndex + 1}/${totalRequired})`}
        </button>
      </div>
    </div>
  );
...
function AddMorePhotos({ requiredPhotos, extraPhotos, onAddMore, onDeleteExtra, onContinue }) {
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <div className="bg-white px-6 pt-10 pb-6 shadow-sm">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-full kfh-bg flex items-center justify-center">
            <Check className="w-5 h-5 text-white" />
          </div>
          <div>
            <h2 className="font-syne text-lg font-bold text-gray-900" style={{fontWeight:700}}>Required Photos Done</h2>
            <p className="text-xs text-gray-400">4 of 4 captured</p>
          </div>
        </div>
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-2xl px-4 py-3 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-orange-700 mb-0.5">Add damage photos?</p>
            <p className="text-xs text-orange-600 leading-relaxed">
              You can add any number of additional photos showing specific damage, interior, or other details to strengthen your claim.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 mt-5">
        ...
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">Required Photos</p>
          ...
          {requiredPhotos.map((photo) => (
            <div key={photo.sideId} className="bg-white rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="px-3 py-2 flex items-center justify-between">
                <span className="text-xs font-medium text-gray-700 truncate">{photo.label}</span>
        ...
        {extraPhotos.length > 0 && (
          <>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">
              Additional Photos ({extraPhotos.length})
            </p>
        ...
        <button onClick={onAddMore}
          ...
          Add {extraPhotos.length > 0 ? 'Another' : 'Additional'} Photo
        </button>
      </div>
      ...
      <div className="px-5">
        ...
        <p className="text-center text-xs text-gray-400 mt-3">
          {extraPhotos.length === 0 ? "No additional photos — you can still submit" : `${extraPhotos.length} additional photo${extraPhotos.length > 1 ? 's' : ''} added`}
        </p>
      </div>
    </div>
  );
...
function ReviewSubmit({ requiredPhotos, extraPhotos, onSubmit, onRetakeSingle, onRetakeExtra, onDeleteExtra, onAddMore, onRetakeAll, isSubmitting }) {
  const allPhotos = [...requiredPhotos, ...extraPhotos];
    ...
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <div className="bg-white px-6 pt-10 pb-6 text-center shadow-sm">
        ...
        <p className="text-gray-500 text-sm">
          {allPhotos.length} photo{allPhotos.length !== 1 ? 's' : ''} total · {requiredPhotos.length} required · {extraPhotos.length} additional
        </p>
      </div>

      <div className="px-5 mt-5 space-y-4">
        {/* Required */}
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Required Photos</p>
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-7 text-center">
      ...
      <div className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-4 fade-up-2">
        ...
        <p className="text-xs text-gray-400 mt-1">Keep this reference to track your claim</p>
      </div>
    </div>
  );
}
...
export default function MotorClaim() {
  ...
  const [requiredPhotos, setRequiredPhotos] = useState([]); // fixed 4
  const [extraPhotos, setExtraPhotos] = useState([]);       // unlimited
  ...
  const handleCapture = (dataUrl) => {
    ...
    if (retakeTarget) {
      if (retakeTarget.type === "required") {
        ...
        updated[retakeTarget.index] = { ...updated[retakeTarget.index], dataUrl };
  ...
  if (screen === "success") return (
    <SuccessScreen reqId={reqId} totalPhotos={requiredPhotos.length + extraPhotos.length} />
  );
}