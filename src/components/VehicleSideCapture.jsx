...
import { ArrowLeft, Camera, Loader2, Wifi, WifiOff, CheckCircle, XCircle, RotateCcw, ChevronRight, AlertCircle } from "lucide-react";
...
/** Safari < 15.4 has no CanvasRenderingContext2D.roundRect — drawing the label without it avoids iOS white-screen crashes. */
...
export default function VehicleSideCapture({
    ...
    const [cameraErrorKey, setCameraErrorKey] = useState(null);
    ...
    useEffect(() => {
        if (currentSideIndex > prevSideIndexRef.current && currentSideIndex <= sideOrder.length) {
            // eslint-disable-next-line react-hooks/set-state-in-effect -- one-shot flash tied to side transition
    ...
    useEffect(() => {
        ...
        acquireCameraStream({
            ...
            .catch((err) => {
                if (!active) return;
                setCameraErrorKey(cameraErrorToTranslationKey(err));
            });
    ...
    const drawBbox = useCallback(() => {
        ...
        try {
            ...
            if (label) {
                ...
                fillRoundRect(ctx, lx, ly, labelW, labelH, 6);
                ctx.fillStyle = "#fff";
                ctx.fillText(label, lx + 8, ly + 17);
            }
        } catch (e) {
            console.warn("[VehicleSideCapture] overlay draw failed:", e);
        }
    }, [bbox, lastResponse, t, videoRef]);
    ...
    return (
      ...
      `}</style>
            ...
            {showLandscapeModal && (
                <div className="absolute inset-0 z-50 flex items-center justify-center p-6 bg-black/80" style={{ paddingBottom: "max(1.5rem, env(safe-area-inset-bottom, 0px))" }}>
                    <div className="bg-gray-900 border border-white/20 p-6 rounded-2xl w-full max-w-sm text-center fade-up shadow-2xl">
                        ...
                        {cameraErrorKey ? (
                            <div className="mb-4 text-left rounded-xl bg-red-500/15 border border-red-500/30 px-3 py-2 flex gap-2 items-start">
                                <AlertCircle className="w-4 h-4 text-red-400 shrink-0 mt-0.5" />
                                <span className="text-red-200 text-xs leading-relaxed">{t(cameraErrorKey)}</span>
                            </div>
                        ) : !streamReady ? (
                        ...
                        {cameraErrorKey ? (
                            <button
                                type="button"
                                onClick={() => setCameraRetryToken((n) => n + 1)}
                                className="w-full py-3 mb-3 bg-white text-black font-bold rounded-xl active:scale-95 transition-transform"
                            >
                                {t("Try again")}
                            </button>
                        ) : null}
